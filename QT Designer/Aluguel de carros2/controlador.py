...
def cadastro_carro():
    ...
    if marca == '' or descricao == '' or placa == '' or preco_diaria == '' or preco_mensal == '' or preco_venda == '':
        ...
        banco.commit()
        cadastrocarros.QLineEdit.clear()
...
def cadastro_cliente():
    ...
    if nome == '' or rg == '' or cpf == '' or email == '' or telefone == '' or endereco == '' or cep == '':
        ...
        cursor.execute(sql, dados)
        banco.commit()
        cadastrousuarios.QLineEdit.clear()
#Lista os carros
def listar_carros():
    listarcarros.show()
...
def consulta(venda):
    ...
    sql += " join cliente on cpf = " + venda.get(cpfcliente)
    sql += "  where placa = " + venda.get(placacarro)
    resultado = ''
    ...
    if resultado == '':
        ...
        coluna = ''
        if venda.categoria["categoria"] == "Aluguel Diario":
            coluna = resultado[0][4]
        elif venda.categoria["categoria"]  == "Aluguel Mensal":
            coluna = resultado[0][5]
        elif venda.categoria["categoria"]  == "Venda":
            coluna = resultado[0][6]

        sql = "INSERT INTO cliente (cpf, placa, valor) VALUES (%s,%s,%s)"
        dados = (str(venda.get(cpfcliente)), str(venda.get(placacarro)), coluna)
        cursor.execute(sql, dados)
        banco.commit()
...
##Telas
telaprincipal = uic.loadUi("telaopcoes.ui")
telavenda = uic.loadUi("telavenda.ui")
cadastrocarros = uic.loadUi("cadastrocarro.ui")
cadastrousuarios = uic.loadUi("cadastrousuario.ui")
listarcarros = uic.loadUi("listadecarros.ui")
listarclientes = uic.loadUi("listadeclientes.ui")
editarcarro = uic.loadUi("editarcarro.ui")
editarcliente = uic.loadUi("editarcliente.ui")
confirmarpagamento = uic.loadUi("confirmarpagamento.ui")
#Ações
...
listarcarros.editar.clicked.connect(editar_cliente)
